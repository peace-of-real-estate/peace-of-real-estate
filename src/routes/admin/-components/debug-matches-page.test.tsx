...
function makePayload(): DebugMatchesPayload {
	const agents = [
		{ agent: makeAgent({ id: 'agent-1' }), name: 'Avery Stone' },
		{
			agent: makeAgent({
				id: 'agent-2',
				city: {
		...
		},
		{
			agent: makeAgent({
				...
				enjoyedClients: ['investors'],
			}),
			name: 'Taylor Reed',
		},
	]

	const scored = agents.map(({ agent, name }, index) =>
		scoredAgent(agent, name, 90 - index * 2, index === 2),
	)
