...
function MatchGeoSlot({
	...
	return (
		<MatchGeoMap
			...
			zipFit={geo.zipFit}
			cityFit={geo.cityFit}
			clientLabel={`${clientProfile.city.name}, ${clientProfile.city.state}`}
			agentLabel={match.location}
			clientCityId={clientProfile.city.id}
			clientZipCodes={clientProfile.geography.map(({ zip }) => zip)}
			agentZipCodes={match.agentProfile.geography.map(({ zip }) => zip)}
		/>
	)
}
