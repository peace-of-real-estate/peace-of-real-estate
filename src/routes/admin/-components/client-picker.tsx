...
interface ClientPickerProps {
	...
	onSelect: (clientId: string, side: ClientRole) => void
...
export function ClientPicker({
	...
	const buyers = options.filter((option) => option.side === 'buyer')
	const sellers = options.filter((option) => option.side === 'seller')
			...
			<PopoverTrigger asChild>
				...
				>
					<span className="truncate">
						{selected
							? `${selected.name ?? selected.email} (${selected.side}) · ${selected.cityName}, ${selected.state}`
							: loading
								? 'Loading clients...'
	...
	return (
		...
		>
			...
			<span className="flex-1 truncate">
				{option.name ?? option.email} · {option.cityName}, {option.state} ·{' '}
				{formatPriceRange(option.priceRange)}
			</span>
