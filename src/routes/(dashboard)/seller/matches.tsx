...
import type { SellerProfile } from '@/lib/matching/profile'
...
function Matches() {
	...
	const { data: matches = [], isLoading } = useQuery({
		...
		queryFn: loadSellerAgentMatches,
	})
	const { data: sellerProfile } = useQuery({
		queryKey: ['seller-profile'],
		queryFn: loadSellerProfile,
	})
	const stateCode = resolveStateCode(sellerProfile?.state ?? undefined)
...
function PreferencesSummaryCard({
	...
	settings: SellerProfile | null | undefined
		...
		<Card className="p-4">
			<div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
				...
				<Button asChild variant="outline" size="sm" className="shrink-0">
					<Link to="/seller/search-preferences">
...
function getPreferenceSummaryItems(profile: SellerProfile | null | undefined) {
	if (!profile) return []
	...
	const answerItems = Object.entries(clientAnswerLabels)
		.map(([id, config]) => {
			const answer = profile[id as keyof SellerProfile] as
				| string
				| string[]
				| undefined
