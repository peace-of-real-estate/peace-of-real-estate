...
import { loadBuyerAgentMatches, loadBuyerProfile } from '@/lib/matching/profile'
import { clientAnswerLabels } from '@/lib/matching/questions'
import type { BuyerProfile } from '@/lib/matching/profile'
...
function Matches() {
	...
	const { data: matches = [], isLoading } = useQuery({
		...
		queryFn: loadBuyerAgentMatches,
	})
	const { data: buyerProfile } = useQuery({
		queryKey: ['buyer-profile'],
		queryFn: loadBuyerProfile,
	})
	const stateCode = resolveStateCode(buyerProfile?.state ?? undefined)
...
function PreferencesSummaryCard({
	...
	settings: BuyerProfile | null | undefined
		...
		<Card className="p-4">
			<div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
				...
				<Button asChild variant="outline" size="sm" className="shrink-0">
					<Link to="/buyer/search-preferences">
...
function getPreferenceSummaryItems(profile: BuyerProfile | null | undefined) {
	if (!profile) return []
	...
	const answerItems = Object.entries(clientAnswerLabels)
		.map(([id, config]) => {
			const answer = profile[id as keyof BuyerProfile] as
				| string
				| string[]
				| undefined
