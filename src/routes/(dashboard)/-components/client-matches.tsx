...
export function ClientMatches({
	...
	const selectableIds = new Set(
		matches
			.filter((match) => {
				const entry = introPayload?.agentStates.find(
					(agentState) => agentState.agentProfileId === match.id,
				)
	...
	const sendMutation = useMutation({
		...
		onSuccess: async (result, agentProfileIds) => {
			setConfirmOpen(false)
			await queryClient.invalidateQueries({
				queryKey: ['client-introductions', clientProfileId],
			})
			if (result.ok) {
				const count = agentProfileIds.length
				toast.success(
					...
					<DialogHeader>
						...
						<DialogDescription>
							{selectedMatches.length === 1
								...
								: "You're about to send introductions to:"}
						</DialogDescription>
					</DialogHeader>
