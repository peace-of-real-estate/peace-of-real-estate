...
export function ClientMatches({
	...
	const selectedMatches = useMemo(
		() => matches.filter((match) => selectedIds.has(match.id)),
		[matches, selectedIds],
	)
...
const COMMUNICATION_LABELS = new Set([
	'Quick chat',
	'Updates & docs',
	'Response time',
...
const BASIC_LABELS = new Set(['Budget', 'Home Type'])

/** Labels whose value gets a small pictograph icon (Text → phone, Email → envelope, …). */
const VALUE_ICON_LABELS = new Set([
	'Location',
	'Home Type',
	'Quick chat',
	'Updates & docs',
...
/** Labels rendered in amber — the traits that drive matching hardest. */
...
function KeyedRow({ item }: { item: SummaryItem }) {
	const signature = SIGNATURE_LABELS.has(item.label)
	const showIcon = VALUE_ICON_LABELS.has(item.label)
	...
	return (
		<div className="border-border border-dashed sm:border-r sm:last:border-r-0">
			...
			<div className="space-y-1.5 p-3">
				{items.map((item) => (
					<KeyedRow key={item.label} item={item} />
				))}
			</div>
		</div>
	)
...
function PreferencesSummaryCard({
	...
	const basicItems = [
		...locationItems,
		...summaryItems.filter((item) => BASIC_LABELS.has(item.label)),
	]
	const communicationItems = summaryItems.filter((item) =>
		COMMUNICATION_LABELS.has(item.label),
	)
	const termsItems = summaryItems.filter((item) => TERMS_LABELS.has(item.label))
	const approachItems = summaryItems.filter(
		(item) =>
			!BASIC_LABELS.has(item.label) &&
			!COMMUNICATION_LABELS.has(item.label) &&
			!TERMS_LABELS.has(item.label),
	)
