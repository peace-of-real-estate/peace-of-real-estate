					...
					>
						<div aria-live="polite" className="text-sm">
							...
							<span className="text-muted-foreground">
								{' '}
								· {Math.max(0, remainingSlots - selectedIds.size)}{' '}
								{remainingSlots - selectedIds.size === 1 ? 'slot' : 'slots'}{' '}
								remaining
							</span>
						</div>
						<Button
...
const DIMENSION_SCORE_KEYS: Record<DimensionId, string> = {
	location: 'Location',
	priceFit: 'Price Fit',
				...
				>
					<div className="w-36 shrink-0">
						<div className="flex items-center gap-1.5">
							...
							{match.isTopMatch && (
								<StarIcon
									className="text-amber h-3 w-3 shrink-0 fill-current"
			...
			<div className="text-muted-foreground flex flex-wrap items-center gap-x-3 gap-y-1 text-xs">
				...
				{match.stats && (
					<span className="flex items-center gap-1">
						...
						{match.stats.transactions} deals/yr
					</span>
				)}
			</div>
