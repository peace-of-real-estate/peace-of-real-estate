...
export type MatchRowState =
	...
	| { kind: 'connected' }
	| { kind: 'cooldown'; retryAt: Date | string | null }
...
function cooldownLabel(retryAt: Date | string | null): string {
	if (!retryAt) return 'Cooldown'
	const time = new Date(retryAt).getTime()
	if (!Number.isFinite(time)) return 'Cooldown'
	const days = Math.max(1, Math.ceil((time - Date.now()) / 86_400_000))
