...
import { agentProfiles, clientProfiles, introductions, user } from '@/db/schema'
	...
	.handler(async ({ data }) => {
		...
		if (result.ok && result.status === 'accepted') {
			try {
				...
				console.error('Failed to retry introduction notifications', error)
			}
		}
		if (result.ok && result.status === 'connected') {
			try {
				...
				console.error('Failed to send connected notification', error)
			}
		}
		return result
	})
