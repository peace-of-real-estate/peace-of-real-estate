import type { ClientRole, ClientWorkStyle } from '@/lib/profile/types'

import type { Introduction } from './db'
import {
	COOLDOWN_MS,
	isActiveStatus,
	isClosedStatus,
...
export function toClientIntroView(
	intro: Pick<Introduction, 'id' | 'status' | 'createdAt' | 'acceptedAt'>,
...
export function buildAgentStates(
	intros: Pick<Introduction, 'agentProfileId' | 'status' | 'closedAt'>[],
	now: Date,
): ClientIntroductionsPayload['agentStates'] {
	const byAgent = new Map<
		string,
		Pick<Introduction, 'agentProfileId' | 'status' | 'closedAt'>[]
	>()
	...
	return [...byAgent.entries()].map(([agentProfileId, rows]) => {
		if (rows.some((row) => isActiveStatus(row.status))) {
		...
		for (const row of rows) {
			...
			if (
				row.closedAt !== null &&
				(latest === undefined || row.closedAt > latest)
			) {
				latest = row.closedAt
			}
		}
