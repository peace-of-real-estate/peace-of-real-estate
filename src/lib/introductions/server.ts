...
import { agentProfiles, clientProfiles, introductions } from '@/db/schema'
...
function logRetryError(label: string) {
...
export const sendIntroductions = createServerFn({ method: 'POST' })
	...
	.handler(async ({ data }) => {
		...
		if (result.ok) {
			...
			// Retry-on-read in the getters stays as the fallback. Failures are
			// logged, never propagated: the introductions are already persisted.
			await retryIntroductionNotifications(db, {
				introductionIds: result.ids,
			}).catch(logRetryError('Introduction notification'))
		}
		return result
	})
...
export const getClientIntroductions = createServerFn({ method: 'GET' })
	...
	.handler(async ({ data }) => {
		...
		}).catch(logRetryError('Connected notification'))
...
export const getAgentIntroductions = createServerFn({ method: 'GET' }).handler(
	async () => {
		...
		void retryConnectedNotifications(db, { agentProfileId: agent.id }).catch(
			logRetryError('Connected notification'),
		)
		void retryIntroductionNotifications(db, { agentProfileId: agent.id }).catch(
			logRetryError('Introduction notification'),
		)
