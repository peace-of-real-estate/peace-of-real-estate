...
async function deliverLifecycleNotifications(
	...
	for (const job of jobs) {
		...
		// Claim, status check, and send share one transaction under the same
		// per-profile advisory lock that withdrawal and acceptance hold, so
		// the status cannot change between the check and the send. Claiming
		// before sending means only the reader that flips sentAt from null
		// may deliver, and a failed send rolls the claim back so the job
		// stays retryable.
		try {
			await db.transaction(async (tx) => {
				await System.lockProfile(tx, row.intro.clientProfileId)
				const claimed = await tx
					.update(introductionNotificationJobs)
					.set({ sentAt: new Date() })
					.where(
						and(
							eq(introductionNotificationJobs.id, job.id),
				...
				const [current] = await tx
					.select({ status: introductions.status })
					.from(introductions)
					.where(eq(introductions.id, job.introductionId))
					.limit(1)
				if (!current) return
				if (kind === 'sent' && current.status === 'pending') {
				...
				} else if (kind === 'accepted' && current.status === 'accepted') {
				...
				} else if (kind === 'declined' && current.status === 'declined') {
				...
				// Any other status means the job is stale (or superseded by the
				// connected notification); committing the claim retires it.
			})
		} catch (error) {
			errors.push(error)
		}
