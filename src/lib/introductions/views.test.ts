...
import { encodeData, type IntroductionData } from './intro-data'
...
function dataFor(status: IntroductionStatus, at: Date = NOW): IntroductionData {
	switch (status) {
		case 'pending':
...
describe('buildAgentStates', () => {
	test('pending and accepted intros map to active', () => {
		expect(
			buildAgentStates(
				[{ agentProfileId: 'a', status: 'pending', data: {} }],
				NOW,
			),
				...
				[
					{
						agentProfileId: 'a',
						status: 'declined',
						data: encodeData.closed(older),
					},
					{
						agentProfileId: 'a',
						status: 'withdrawn',
						data: encodeData.closed(recent),
					},
				],
				NOW,
			),
