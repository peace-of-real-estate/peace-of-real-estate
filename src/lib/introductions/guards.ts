...
export const COOLDOWN_MS = 30 * 86_400_000
export const WITHDRAW_MIN_AGE_MS = 24 * 3_600_000
...
export function checkCooldown(
	...
	if (remainingMs > 0) {
		return {
			code: 'COOLDOWN',
			message: `Wait ${Math.ceil(remainingMs / 86_400_000)} more day(s).`,
		}
	}
	return null
}
...
export function checkWithdrawable(
	...
	if (remainingMs > 0) {
		return {
			...
			message: `You can withdraw in ${Math.ceil(remainingMs / 3_600_000)} more hour(s).`,
		}
	}
	return null
}
