...
import { decodeData, encodeData } from './intro-data'
import type { IntroAccessWindow, IntroductionStatus } from './types'
...
const ACTIVE_STATUSES: IntroductionStatus[] = ['pending', 'accepted']
const PAIR_BLOCKING_STATUSES: IntroductionStatus[] = [
	'pending',
	'accepted',
	'connected',
]
	...
	async send(
		...
		return db.transaction(async (tx) => {
			...
			const agents = await tx
				.select()
				.from(agentProfiles)
				.where(inArray(agentProfiles.id, agentProfileIds))
			...
			const side = transactionSide(profile.role)
			const now = new Date()
...
export const Agent = {
	...
	async list(db: Db, agentProfileId: string): Promise<AgentIntroView[]> {
		...
		const flatProfiles = new Map<string, ClientProfileRow | undefined>()
		for (const row of rows) {
		...
		return rows.map((row) => {
			const side = transactionSide(row.profile.role)
			const fit = calculateFitScore(
				agent,
				flatProfiles.get(row.profile.id),
				side,
			)
			return toAgentIntroView(row.intro, {
				...
				priceRange: formatPriceRange(parsePriceRange(row.profile.priceRange)),
...
async function loadFlatProfile(
	...
	return profile.role === 'buyer'
		? loadBuyerProfileById(profile.id, executor)
		: loadSellerProfileById(profile.id, executor)
}
